...
-(NSDictionary*) getHeaders:(NSData*) data
{
    ...
    if ([headers count] <=0 )
        return nil;
    NSMutableDictionary *resultHeaders = [[NSMutableDictionary alloc] init];
    ...
    if (READ_TAG_HTTP_HEADERS == tag )
    {
        ...
        else
        {
            if ([imgBuffer length] > MAX_FRAME_SIZE)
            {
                ...
                NSError *error = [[NSError alloc] initWithDomain:NSCocoaErrorDomain code:ERROR_UNKNOWN userInfo:userInfo];
                if (_delegate)
                    [_delegate mjpegClient:self didReceiveError:error];
                [self stop];
                return;
